...
Axios.interceptors.response.use(
  ...
  (response) => {
    if (response && response.statusCode === 500 && response.data.message) {
      notification.error({
        message: "Thông báo",
        description: response.data.message,
      });
    }
    if (response && response.statusCode === 200 && response.data.message) {
      notification.success({
        message: "Thông báo",
        description: response.data.message,
      });
    }
    // trả về 1 cái response
    return response;
  },
...
export async function postData({ url, payload, method }) {
  try {
    ...
    if (res && res.data) {
      localStorage.setItem("ex1-auth-token", res.data.message);
    }
  } catch (err) {}
}
